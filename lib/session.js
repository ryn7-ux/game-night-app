...
export async function revealPartnerAnswers() {
      const snap = await get(ref(db, "session/partnerGame"));
      const data = snap.val();
      if (!data) return;
      ...
      Object.values(pairs).forEach((pair) => {
              const aAns = answers[pair.a];
              const bAns = answers[pair.b];
              if (aAns && bAns) {
                        if (norm(aAns.guess) && norm(aAns.guess) === norm(bAns.own)) {
                                    awards.push(addLeaderboardPoints(pair.a, 1));
                        }
                        if (norm(bAns.guess) && norm(bAns.guess) === norm(aAns.own)) {
                                    awards.push(addLeaderboardPoints(pair.b, 1));
                        }
              }
      });
