...
export async function playerExists(id) {
  const snap = await get(ref(db, `session/players/${id}`));
  return snap.exists();
}
...
// Finds an existing player whose name exactly or closely matches (typo-tolerant),
// so someone who lost local storage (new device, cleared cache, in-app browser)
// can rejoin their existing player record instead of starting over at 0 points.
export async function findClosestPlayerName(name) {
  const normalized = name.trim().toLowerCase();
  if (!normalized) return null;
  const snap = await get(ref(db, "session/players"));
  const players = snap.val() || {};
  let best = null;
  let bestDist = Infinity;
  for (const [id, data] of Object.entries(players)) {
    const otherName = (data.name || "").trim().toLowerCase();
    if (!otherName) continue;
    if (otherName === normalized) {
      return { id, name: data.name, avatarId: data.avatarId, distance: 0 };
    }
    const dist = levenshtein(normalized, otherName);
    const threshold = otherName.length <= 5 ? 1 : 2;
    if (dist <= threshold && dist < bestDist) {
      best = { id, name: data.name, avatarId: data.avatarId, distance: dist };
      bestDist = dist;
    }
  }
  return best;
...
// Compares an archived old game's players against the current live players by name,
// so the host can merge an old game's scores into the currently active one.
export async function previewImportFromArchive(archiveId) {
  const archiveSnap = await get(ref(db, `archives/${archiveId}`));
  const archive = archiveSnap.val();
  if (!archive) return null;
  const oldPlayers = archive.players || {};
  const oldLeaderboard = archive.leaderboard || {};

  const currentSnap = await get(ref(db, "session/players"));
  const currentPlayers = currentSnap.val() || {};
  const currentList = Object.entries(currentPlayers).map(([id, data]) => ({
    id,
    name: data.name || "",
  }));
  ...
  for (const [oldId, oldData] of Object.entries(oldPlayers)) {
    ...
    for (const cur of currentList) {
      ...
      const dist = levenshtein(oldName, curName);
      const threshold = curName.length <= 5 ? 1 : 2;
      if (dist <= threshold && dist < bestDist) {
        best = cur;
        bestDist = dist;
      }
    ...
    const entry = {
      ...
      oldAvatarId: oldData.avatarId,
      oldScores: oldLeaderboard[oldId] || {},
    };
    if (best) {
      matches.push({ ...entry, newId: best.id, newName: best.name });
    } else {
      unmatched.push(entry);
    }
  }

  return { matches, unmatched, currentList };
}

// pairings: [{ oldId, oldName, oldAvatarId, oldScores, action: "merge" | "new", newId? }]
// "merge" adds the old game's per-game points on top of whatever the matched player
// already has this game; "new" brings them in fresh with their old scores intact.
export async function applyImportFromArchive(pairings) {
  for (const p of pairings) {
    if (p.action === "merge" && p.newId) {
      const entries = Object.entries(p.oldScores || {});
      for (const [gameId, pts] of entries) {
        await runTransaction(ref(db, `session/leaderboard/${p.newId}/${gameId}`), (current) => {
          return (current || 0) + (pts || 0);
        });
      }
    } else {
      const newId = "p_" + Math.random().toString(36).slice(2, 10);
      await set(ref(db, `session/players/${newId}`), {
        name: p.oldName,
        avatarId: p.oldAvatarId || null,
        joinedAt: Date.now(),
      });
      const entries = Object.entries(p.oldScores || {});
      for (const [gameId, pts] of entries) {
        await set(ref(db, `session/leaderboard/${newId}/${gameId}`), pts || 0);
      }
    }
  }
}

export async function getOrCreateGameCode() {
  const snap = await get(ref(db, "session/gameCode"));
  let code = snap.val();
  if (!code) {
    code = "g_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8);
    await set(ref(db, "session/gameCode"), code);
  }
  return code;
}

export function getStoredGameCode() {
  if (typeof window === "undefined") return null;
  return localStorage.getItem("gamenight_gameCode");
}

export function setStoredGameCode(code) {
  if (typeof window === "undefined") return;
  localStorage.setItem("gamenight_gameCode", code);
}
...
export function listenArchives(callback) {
  return onValue(ref(db, "archives"), (snap) => {
    ...
    const list = Object.entries(val)
      .map(([id, data]) => ({ id, label: data.label, archivedAt: data.archivedAt, auto: !!data.auto, trashed: !!data.trashed, trashedAt: data.trashedAt || null }))
...
export async function trashArchive(id) {
  await update(ref(db, `archives/${id}`), { trashed: true, trashedAt: Date.now() });
}

export async function restoreArchive(id) {
  await update(ref(db, `archives/${id}`), { trashed: false, trashedAt: null });
}

// Anything sitting in Trash for 45+ days gets permanently deleted. Called
// opportunistically whenever the host page loads the archive list (no server
// cron in this setup, so this "lazy sweep" runs on whoever next opens Host).
const TRASH_RETENTION_MS = 45 * 24 * 60 * 60 * 1000;
export async function purgeExpiredTrash(archives) {
  const now = Date.now();
  const expired = (archives || []).filter(
    (a) => a.trashed && a.trashedAt && now - a.trashedAt > TRASH_RETENTION_MS
  );
  await Promise.all(expired.map((a) => removeArchive(a.id)));
  return expired.length;
}

export async function archiveCurrentSession(label, isAuto) {
...
export async function startNewGame(label) {
  await archiveCurrentSession(label, false);
  await remove(ref(db, "session"));
  const code = "g_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8);
  await set(ref(db, "session/gameCode"), code);
}
...
export async function loadArchivedSession(archiveId) {
  ...
  const data = snap.val();
  if (!data) return;
  await archiveCurrentSession("Auto-backup before restore", true);
  const { archivedAt, label, auto, ...sessionData } = data;
  await set(ref(db, "session"), sessionData);
}

// --- Casino Finale (end-of-night roulette) ---

export function listenCasino(callback) {
  return onValue(ref(db, "session/casino"), (snap) => {
    callback(snap.val() || { status: "idle" });
  });
}

export async function endGameNight() {
  await set(ref(db, "session/casino"), {
    status: "leaderboard",
  });
}

export async function setCasinoStatus(status, extra) {
  await update(ref(db, "session/casino"), { status, ...(extra || {}) });
}

export async function placeCasinoBet(playerId, bet) {
  await set(ref(db, `session/casino/bets/${playerId}`), bet);
}

export async function setCasinoResult(playerId, result) {
  await set(ref(db, `session/casino/results/${playerId}`), result);
}

export async function resetCasino() {
  await set(ref(db, "session/casino"), { status: "idle" });
}

// --- Who Sent This? photo bank (pre-staged submissions) ---

export function listenWhoSentBank(callback) {
  return onValue(ref(db, "whoSentBank"), (snap) => {
    callback(snap.val() || {});
  });
}

export async function addWhoSentBankEntry(imageUrl, senderName) {
  const key = `wsb_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
  await set(ref(db, `whoSentBank/${key}`), {
    imageUrl,
    senderName,
    addedAt: Date.now(),
    used: false,
...
export async function removeWhoSentBankEntry(key) {
  await remove(ref(db, `whoSentBank/${key}`));
}

export async function markWhoSentBankUsed(key) {
  await update(ref(db, `whoSentBank/${key}`), { used: true });
}

export function listenGuessClue(callback) {
  return onValue(ref(db, "session/guessClue"), (snap) => {
    callback(snap.val() || null);
  });
}

export async function pushGuessClue(question, answerText, subjectName) {
  await set(ref(db, "session/guessClue"), {
    question,
    answerText,
    subjectName,
    startedAt: Date.now(),
...
export async function submitGuessClueGuess(playerId, guessText) {
  await set(ref(db, `session/guessClue/guesses/${playerId}`), guessText);
}

export async function markGuessClueCorrect(playerId, isCorrect) {
  await set(ref(db, `session/guessClue/correct/${playerId}`), isCorrect);
}

export async function revealGuessClue() {
  const snap = await get(ref(db, "session/guessClue"));
  const data = snap.val();
  if (!data) return;
  const correct = data.correct || {};
  const correctIds = Object.entries(correct)
    .filter(([_, v]) => v)
    .map(([id]) => id);
  await Promise.all(correctIds.map((id) => addGameScore("whoSent", id, 1)));
  await update(ref(db, "session/guessClue"), { revealed: true });
}

export async function clearGuessClue() {
  await remove(ref(db, "session/guessClue"));
}

export function listenGuessClueBank(callback) {
  return onValue(ref(db, "guessClueBank"), (snap) => {
    callback(snap.val() || {});
  });
}

export async function addGuessClueBankEntry(question, answerText, subjectName) {
  const key = `gc_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
  await set(ref(db, `guessClueBank/${key}`), {
    question,
    answerText,
    subjectName,
    addedAt: Date.now(),
...
export async function removeGuessClueBankEntry(key) {
  await remove(ref(db, `guessClueBank/${key}`));
}

export async function markGuessClueBankUsed(key) {
  await update(ref(db, `guessClueBank/${key}`), { used: true });
}
