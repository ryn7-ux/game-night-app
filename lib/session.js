...
export function listenArchives(callback) {
  return onValue(ref(db, "archives"), (snap) => {
    ...
    const list = Object.entries(val)
      .map(([id, data]) => ({ id, label: data.label, archivedAt: data.archivedAt, auto: !!data.auto, trashed: !!data.trashed }))
...
export async function trashArchive(id) {
  await update(ref(db, `archives/${id}`), { trashed: true });
}

export async function restoreArchive(id) {
  await update(ref(db, `archives/${id}`), { trashed: false });
}
